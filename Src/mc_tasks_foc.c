      ...
      switch (Mci[M1].State)
      {
        ...
        case IDLE:
        {
          if ((MCI_START == Mci[M1].DirectCommand) || (MCI_MEASURE_OFFSETS == Mci[M1].DirectCommand))
          {
            ...
            {
              if (true == FOCVars[M1].flagStartWithChargeBootCap)
              {
        ...
        case CHARGE_BOOT_CAP:
        {
          ...
          else
          {
            ...
            {
              if (FOCVars[M1].flagStartWithChargeBootCap == false)
              {
                 FOC_Clear(M1);
                 FOCVars[M1].flagStartWithChargeBootCap = true;
                 Mci[M1].State = IDLE;
              }
              else
              {
