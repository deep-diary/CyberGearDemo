...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	CAN_SendResponseCmdType2(0xfd, 0x01); // 应答使能状态
  while (1)
  {
...
static void MX_FDCAN1_Init(void)
{
  ...
  /* USER CODE BEGIN FDCAN1_Init 2 */
    ...
    FDCAN1_RXFilter.FilterType=FDCAN_FILTER_RANGE;                   //滤波器类�??
    FDCAN1_RXFilter.FilterConfig=FDCAN_FILTER_TO_RXFIFO0;           //过滤�??0关�?�到FIFO0  
    FDCAN1_RXFilter.FilterID1=0x0000;                               //32�???ID
    FDCAN1_RXFilter.FilterID2=0x0000;                               //如果FDCAN�???�?为传统模�??的�?，这里是32�???掩�?
    if(HAL_FDCAN_ConfigFilter(&hfdcan1,&FDCAN1_RXFilter)!=HAL_OK) //滤波�?�??始化
		{
			Error_Handler();
		}
...
static void MX_TIM1_Init(void)
{
  ...
  */
  ...
  LL_GPIO_Init(M1_DP_GPIO_Port, &GPIO_InitStruct);
